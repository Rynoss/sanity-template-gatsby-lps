...
import { Select, TextInput, Autocomplete, Radio, Grid, Card, Text, Flex, Box, Avatar } from '@sanity/ui';
...
const Icon = React.forwardRef((props, ref) => { //creates react component
    const { 
    ...
    useEffect(() => {
      loadStyle("https://d1azc1qln24ryf.cloudfront.net/198172/ryno-lp-factory/style-cf.css")
    ...
    async function getIcons() {
      const iconPackSelection = await (await fetch('https://d1azc1qln24ryf.cloudfront.net/198172/ryno-lp-factory/selection.json')).json();
        ...
        >
          <Card padding={[0, 0, 0]} paddingBottom={[0, 0, 0]}>
            <Autocomplete
              ...
              filterOption={(query, icon) => // custom search filter
                icon.value
                  .toLowerCase()
              ...
              renderOption={(option) => ( // custom option render function
                <Card as="button">
                  <Flex align="center">
